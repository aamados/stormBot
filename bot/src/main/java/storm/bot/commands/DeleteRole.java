...
public class DeleteRole implements Commands
{
	...
	public void executeCommand(GuildMessageReceivedEvent e) 
	{
		...
		{
			util.sendErrorMessage("Usage: .delRole <ROLE> <@USER> OR .delRole <ROLE>", e);
			return;
		}
		...
		if (args.length == 2)
		{
			...
			try
			{
				e.getGuild().removeRoleFromMember(sender, role).complete();
			}
			...
			util.sendMessage("Removed the " + args[1] + " role to yourself.", e);
			return;
		}
		...
		if (args.length == 3)
		{
			...
			try
			{
				e.getGuild().removeRoleFromMember(user, role).complete();
			}
			catch(HierarchyException errorEvent)
			{
				util.sendErrorMessage(errorEvent.getLocalizedMessage(), e);
			...
			util.sendMessage("Removed the " + args[1] + " role to " + user.getNickname(), e);
		}
	}
	...
	{
		return "Deletes the role of a given user.";
	}
}
